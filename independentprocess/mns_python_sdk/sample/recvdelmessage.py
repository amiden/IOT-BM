...
queue_name = 'aliyun-iot-a1bxPiJvYWZ'#sys.argv[1] if len(sys.argv) > 1 else "MySampleQueue"

my_queue = my_account.get_queue(queue_name)
my_queue.set_encoding(True)
...
while True:
    ...
    try:
        ...
        try:
            #recv_msg.message_body.payload = base64.b64decode(recv_msg.message_body.payload)
            ...
            if json.loads(recv_msg.message_body)['messagetype'] == 'status':
                print base64.b64decode(json.loads(recv_msg.message_body)['payload'])
            if json.loads(recv_msg.message_body)['messagetype'] == 'upload':
                print base64.b64decode(json.loads(recv_msg.message_body)['payload'])
        except Exception ,e:
            print Exception,e
            pass
